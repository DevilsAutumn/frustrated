...
export function FrustrationFeed({ items, onReact }: FrustrationFeedProps) {
  if (items.length === 0) {
    return (
      <div className="grid gap-3">
        <div className="grid min-h-56 place-items-center rounded-lg border border-zinc-800 bg-zinc-950/80 p-8 text-center">
  ...
  return (
    <div className="grid gap-3">
      {items.map((item) => (
        <article
          className="rounded-lg border border-zinc-800 bg-zinc-950/80 p-4 transition hover:border-zinc-700"
          key={item.id}
        >
